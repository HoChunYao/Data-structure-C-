...
class LinkedQueue{
struct LinkedNode* front = NULL;
struct LinkedNode* rear = NULL;
struct LinkedNode* temp;
public:
// function for add a data value in queue.
void Insert(int val) {
    if (rear == NULL) {
        rear = (struct LinkedNode *)malloc(sizeof(struct LinkedNode));
        rear->next = NULL;
        rear->data = val;
        front = rear;
    }
    else {
        temp=(struct LinkedNode *)malloc(sizeof(struct LinkedNode));
        rear->next = temp;
        temp->data = val;
        temp->next = NULL;
        rear = temp;
...
int Delete() {
    ...
    if (front == NULL) {
        ...
        temp = temp -> next;
        //cout << "\nThe deleted value is : " << front -> data << endl;
        return_value = front -> data;
        free(front);
        front = temp;
        return return_value;
    }
...
void Display(fstream &output) {
    ...
    if ((front == NULL) && (rear == NULL)) {
        cout << "\nQueue is empty, choose[1] to add value to the queue. \n";
        return;
    }
    //cout << "\nQueue elements are : ";
    while (temp != NULL) {
...
void processFile(ifstream& input)
{
    ...
    while(!input.eof())
    {
        ...
        if(tokens.size() == 3){
           ...
           //cout<<structure_name2<<" "<<operation<<" "<<num<<endl;
        ...
        string ch;
        ifstream readOutfile("output.txt");
        cout<<"output file " <<endl;
        while(!readOutfile.eof()){
        getline(readOutfile,ch);
        cout<<ch<<endl;
        }
}
...
int main()
{
    ...
    string whichFile;
    do{
        cout<<"Enter the file you want to choose  "<<endl;
        cout<<"1. file from sample one "<<endl;
        cout<<"2. file from sample two "<<endl;
        cout<<"3. file create by ourselves "<<endl;
        cout<<"Please enter the number from (1,2,3) : " ; cin>>choice;
        if(choice == 1)
            whichFile = "input file.txt";
        else if(choice == 2)
            whichFile = "input file2.txt";
        else if(choice == 3)
            whichFile = "input file3.txt";
        else
